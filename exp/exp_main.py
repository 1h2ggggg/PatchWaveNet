...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, PatchWaveNet
...
class Exp_Main(Exp_Basic):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if 'Linear' in self.args.model or 'TST' in self.args.model or 'WaveNet' in self.args.model or 'P_' in self.args.model:
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    train_loss.append(loss.item())
                # if i % 10 == 0:
                #     print(f"Epoch {epoch + 1}, Iter {i}, adp max: {adp.max()}, min: {adp.min()}")
                #     adp = adp.detach().cpu().numpy()
                #     plt.figure()
                #     sns.heatmap(adp, cmap="viridis", annot=True)
                #     plt.title(f"Epoch {epoch + 1}, Iter {i}", fontsize=20)
                #     plt.savefig(os.path.join(heatmap_dir, f"epoch{epoch + 1}_iter{i}.pdf"))
                #     plt.show()
                # if (i + 1) % 100 == 0:
        ...
        # plt.plot(range(1, len(vali_losses) + 1), vali_losses, label="Vali Loss", marker='^')
        # plt.axvline(x=len(train_losses) - 20, color='r', linestyle='--', label='Early Stop')
    ...
    def test(self, setting, test=0):
        ...
        # print('true', true_series.shape)
        # plt.figure()
        # plt.plot(true_series, label="GroundTruth", linewidth=2)
        # plt.plot(pred_series, label="Prediction", linewidth=2)
        # plt.legend()
        # plt.xlabel("Time Step")
        # plt.ylabel("Value")
        # plt.savefig(os.path.join(folder_path, 'full_prediction.pdf'),bbox_inches='tight')
        ...
        print('mse:{}, mae:{}, rse:{}, mape:{}, rmse:{}'.format(mse, mae, rse, mape, rmse))
        f = open("result.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{}, mae:{}, rse:{}, mape:{}, rmse:{}'.format(mse, mae, rse, mape, rmse))
