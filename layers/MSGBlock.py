...
class GraphBlock(nn.Module):
    def __init__(self, c_out, d_model, conv_channel, skip_channel,
                        gcn_depth, dropout, propalpha, seq_len, node_dim):  # node_dim = 10
    ...
    def forward(self, x):           # 每个变量为一个节点
        adp = F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)), dim=1)   # 构造一个邻接矩阵 [c_out, c_out]
        out = x.unsqueeze(1).transpose(2, 3)   # [B, T, d_model]->[B,1,T, d_model]->[B, 1, d_model, T]
        out = self.start_conv(out)  # [B,conv_channel, c_out,T]
        out = self.gelu(self.gconv1(out, adp))   # [B,skip_channel, c_out, T]
        out = self.end_conv(out).squeeze()  # [B,T,c_out,1]->[B,T,c_out]
        out = self.linear(out)  # [B,T,c_out]->[B,T,d_model]
...
class nconv(nn.Module):
    ...
    def forward(self,x, A):
        x = torch.einsum('ncwl,vw->ncvl',(x,A))  # [B, conv_channel, c_out,T]*[c_out,c_out] = [B, conv_channel,c_out,T]
...
class mixprop(nn.Module):
    ...
    def forward(self, x, adj):
        adj = adj + torch.eye(adj.size(0)).to(x.device)  # 邻接矩阵+单位阵=度矩阵
        d = adj.sum(1)  # 计算每个节点度数
        h = x   # [B, conv_channel, c_out,T]
        out = [h]
        a = adj / d.view(-1, 1)  # (邻接矩阵+单位阵)/节点度数
        for i in range(self.gdep):
            h = self.alpha*x + (1-self.alpha)*self.nconv(h,a)  # [B, conv_channel,c_out,T]
            out.append(h)
        ho = torch.cat(out,dim=1)   # [B,3*conv_channel, c_out, T]
        ho = self.mlp(ho)   # [B,3*conv_channel, c_out, T]->[B,skip_channel, c_out, T]
        return ho
